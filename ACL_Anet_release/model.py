...
class MDC(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3):
        ...
        # kernel_size = 3
        self.conv1 = nn.Conv1d(in_channels=in_ch, out_channels=out_ch // 4, kernel_size=kernel_size, dilation=1,
                               stride=1,
                               padding=1,
                               groups=1)
        self.conv2 = nn.Conv1d(in_channels=in_ch, out_channels=out_ch // 4, kernel_size=kernel_size, dilation=2,
                               stride=1,
                               padding=2,
                               groups=1)
        self.conv3 = nn.Conv1d(in_channels=in_ch, out_channels=out_ch // 4, kernel_size=kernel_size, dilation=3,
                               stride=1,
                               padding=3,
                               groups=1)
        self.conv4 = nn.Conv1d(in_channels=in_ch, out_channels=out_ch // 4, kernel_size=1, dilation=3,
                               stride=1,
                               padding=0,
                               groups=1)

    def forward(self, inputs):
        inputs_1 = F.relu(self.conv1(inputs))
        inputs_2 = F.relu(self.conv2(inputs))
        inputs_3 = F.relu(self.conv3(inputs))
        inputs_4 = F.relu(self.conv4(inputs))
        inputs = torch.cat((inputs_1, inputs_2, inputs_3, inputs_4), 1)
        return inputs
